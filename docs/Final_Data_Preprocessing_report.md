...
### 2.2 개별 데이터 소스별 수집 및 전처리 정제 규칙

*   **식물 마스터 데이터 (`build_plant_master.py` 기반)**
    *   **기준 대상**: `priority_plant_catalog.jsonl` 시드 데이터.
    *   **주요 필드**: `plant_id`, `name_ko`, `name_scientific`, `name_en`, `aliases`, `family`, `category`, `description` 등.
    *   **정제 규칙**: 사용자 검색/등록/자동완성 대응용 식물 마스터 카탈로그 구축 (RAG 지식 본문과 완전히 물리 분리하여 조회 속도 최적화).
*   **농사로 실내정원 식물 데이터**
    *   **대상 범위**: 식물명, 과명, 생육 온도, 습도, 광도, 물주기, 관리 난이도, 생육형태, 잎색, 병충해 코드, 배치 장소.
    *   **정제 규칙**: 일부 코드 필드에 대한 API 설명 누락을 극복하기 위해, 수집된 상세코드 값과 HWP 문서 상세코드표를 크로스 매핑 처리.
*   **농사로 작물 데이터**
    *   **대상 범위**: 작물명, 작물 코드, 학명/영명, 재배환경, 물관리, 광도/햇빛, 온도, 생육단계, 병해충 및 예방/관리 방법.
    *   **수집 전략**: `cropEbook API` + 웹 크롤링(`farmTechMain` 상세) + 농업기술길잡이(농작업 일정 HWPX) 교차 결합.
*   **농사로 농작업 일정 HWPX 파싱**
    *   **파싱 기술**: HWPX 내부 XML 문단 및 표(Table) 셀 단위 디코딩 적용.
    *   **정형화 규칙**:
        *   *정형 표*: RAG가 자연스럽게 문맥을 형성하도록 문장형(Text) 흐름으로 설명 병합.
        *   *비정형 표*: 행/열 의미 구조를 깨뜨리지 않도록 줄바꿈 및 행 구조 형태 보존.
        *   표 내부 문단 중복 제거 및 기존 메타데이터 유지 기술 적용.
*   **네이버 지식백과 보조 문서**
    *   **URL 검수**: API 추출 대상 중 사람의 수동 검증(`manual_url/use/review_status`)을 거친 신뢰할 수 있는 웹 페이지 선별.
    *   **크롤링 가공**: Playwright 동적 렌더링 기반 본문 수집 ➡️ 식물별 그룹화 ➡️ RAG 설명 보강 텍스트로 가공.
*   **NCPMS 병해충 참고 데이터**
    *   **세부 API**: 병 상세(SVC05), 해충 상세(SVC07), 상담 상세(SVC42).
    *   **태깅 규칙**: 본문 내 농약/방제 문구 감지 시 `pesticide_caution` 자동 태깅, 전문가 상담 데이터는 `expert_case_reference`로 별도 격리 분류.
*   **PSIS 농약 안전 정보**
    *   **적용 범위**: 안전성 참고 문서(`safety_reference_only`)로만 분류하고 RAG의 직접적인 처방 근거로 사용 금지.
    *   **강제 안내**: 등록 농약 여부 확인, 라벨/안전사용기준 준수, 전문가 상담 안내 문구 자동 노출용 원천 정보화.
*   **AI Hub 라벨 메타데이터**
    *   **가공 범위**: 이미지 배제, 관수/수분/센서 상태 요약 및 육묘 작물 생육단계 라벨 JSON 텍스트 정보만 RAG 보조 문서로 제한.
    *   **RAG 사용**: 일반 사용자 식물 관찰 및 환경 위해 대응 보조 context 용도 활용.
